...
function find_all_indices_close_to_value(array: number[], value: number, eps: number = 1e-1): number[] {
...
export class PCPAxesMoveToolView extends GestureToolView {
  ...
  get move_axis(): Axis | null {
    return this.idx_move_axis != null ? <Axis>this.plot_model.right[this.idx_move_axis] : null;
  }

  get lines_data_source(): ColumnDataSource {
    const renderer = <GlyphRenderer>this.plot_model.renderers.filter((r) => r.name == "pcp_lines_renderer")[0];
    return <ColumnDataSource>renderer.data_source;
  }

  get selection_data_source(): ColumnDataSource {
    const renderer = <GlyphRenderer>this.plot_model.renderers.filter((r) => r.name == "pcp_selection_renderer")[0];
    return <ColumnDataSource>renderer.data_source;
  }
  ...
  _pan_start(ev: PanEvent): void {
    ...
    const dist_click_axes = this.axes.map((axis) => Math.abs(<number>axis.fixed_location! - x_click_position));
    this.idx_move_axis = Math.min(...dist_click_axes) < 0.25 ? argmin(dist_click_axes) : null;
  ...
  _pan(ev: PanEvent): void {
    if (this.move_axis != null) {
      ...
      this.idx_boxes_on_axis.forEach((idx) => (this.x_selection_boxes[idx] = current_x));
      if (Math.abs(current_x - this.old_axis_location) > 1) {
        ...
        const shift_number = Math.floor(Math.abs(current_x - this.old_axis_location));
        if (shift_sign == -1 && this.old_axis_location > 0) {
          ...
          this.axes
            .filter((axis) => axis.fixed_location! > current_x && axis.fixed_location! < this.old_axis_location)
          ...
          const new_xs = sort_indices.map((idx) => {
            ...
            if (current_x < __xsi && __xsi < this.old_axis_location) return __xsi + 1;
            else return __xsi;
          });
          ...
          if (this.model.pcp_selection_tool != null) this.model.pcp_selection_tool.invalidate_selection = true;
        } else if (shift_sign == 1 && this.old_axis_location < this.axes.length - 1) {
          ...
          const new_xs = sort_indices.map((idx) => {
            const __xsi = __xs0[idx];
            if (this.old_axis_location < __xsi && __xsi < current_x) return __xsi - 1;
            else return __xsi;
          });
          inplace_map(this.__xs, (_) => new_xs);
          inplace_map(this.__ys, (item) => sort_indices.map((idx) => item[idx]));
          ...
          this.idx_data_xs = Math.min(this.idx_data_xs + shift_number, this.__xs.length - 1);
  ...
  _pan_end(_e: PanEvent): void {
    if (this.idx_move_axis != null) {
      ...
      const new_xs = this.__xs[0].map((__xs0i) => {
        if (__xs0i == this.move_axis!.fixed_location) return this.old_axis_location;
        else return __xs0i;
      });
      inplace_map(this.__xs, (_) => new_xs);
      this.move_axis!.fixed_location = this.old_axis_location;
      const x_boxes = this.selection_data_source.get_array("x");
      this.idx_boxes_on_axis.forEach((idx) => (x_boxes[idx] = this.move_axis!.fixed_location));
      this.lines_data_source.change.emit();
      this.selection_data_source.change.emit();
      if (this.model.pcp_selection_tool != null) this.model.pcp_selection_tool.invalidate_selection = true;
