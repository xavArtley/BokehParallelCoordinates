...
export class PCPTicker extends Ticker {
  ...
  get_ticks(data_low: number, data_high: number, _range: Range, _cross_loc: number): TickSpec<number> {
    const major = this.pcp_axes.filter(
      axis => axis.fixed_location!>data_low && axis.fixed_location!<data_high
    ).map(
      axis => <number>axis.fixed_location!
    )

    return {
      // axis.fixed_location>_data_low && axis.fixed_location<_data_high).map(axis => axis.fixed_location)],
